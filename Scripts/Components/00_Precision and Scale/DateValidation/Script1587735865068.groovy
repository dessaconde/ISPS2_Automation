...
'Verify if the error message will display when the past date is entered'
if (PastDtValidation == 'Y') {
	
	'Wait for 5sec'
	WebUI.delay(5)
	
	'Set the date to past date'
	GetDate = (date - 30)
	
	'Convert the format of the past date'
	PastDate = GetDate.format('dd/MM/yyyy')
	
	'Enter the past date'
	WebUI.setText(DateFieldNm, PastDate)
	
	'Hover over the date field'
    WebUI.mouseOver(findTestObject('Objects.DateValidation/Page_ISPS2/tooltipErrDate'), FailureHandling.STOP_ON_FAILURE)

	'Get the error message for entering past date'
    PastDtErrMsg = WebUI.getAttribute(findTestObject('Objects.DateValidation/Page_ISPS2/tooltipErrDate'), 'data-errorqtip', 
        FailureHandling.STOP_ON_FAILURE)
    ...
    ExpectedErrDtMsg = WebUI.concatenate(GlobalVariable.DtErrMsg, ' ', TodayDate)
